...
> 本目录提供 paired-trading 引擎的开发指导文档，理论基础包含宋建毅《K 线动能理论》、Al Brooks 价格行为理论与肖淳心飞天期权择时体系。
> **本目录不包含可执行代码**——它是供 paired-trading 引擎参考实现的工程规范。
...
理论概念与方法论的人类可读版本在 `~/wiki/option-timing/macd-momentum/`（wiki 目录保留旧名，记录 K 线动能理论部分）：
