...
## 当前状态快照（commit `0f940f46`，origin/main 已同步）

- **Baselines dashboard**: **10 OK / 1 STALE**（仅 `pa_h2_climax` STALE/weight-0）。无 PENDING。
...
| 项目 | 说明 | 起点 |
|------|------|------|
| **共享 harness 边界 bug** | Codex 在 pa_atop 发现 `simulate_short` 的 TP1-at-boundary 计分 bug（已修 pa_atop）。**同一 latent bug 在所有 bottom harness 的 `simulate_trade` 里**。修了会微调 historical baseline 数字 → 需重验受影响 baseline。 | `backtest_pa_standalone.py:130-147` 等；参照 `backtest_pa_atop.py` 的修法 |
