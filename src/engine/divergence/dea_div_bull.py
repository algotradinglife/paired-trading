...
Symmetrical to DEAD (dea_div.py) but for the DIF > 0 half of the cycle.
These are bottom signals occurring during a bullish DIF phase — price dips
while DEA temporarily pulls back (slope negative) then resumes.

DEA = DIF − Histogram/hist_scale, derived internally from the inputs.

Pattern:
  - DIF > 0 (current cycle is bullish)
  - Within the last LOOKBACK bars, DEA reached a local trough T (still
    positive since DIF > 0) and slope was negative after T
  - Current DEA slope (dea[i] − dea[i−SLOPE_WINDOW]) has crossed ≥ 0
  - Current bar's low is LOWER than the trough bar's low
  → Price/DEA divergence: signal-line pullback momentum ending while price dips
...
def detect_dea_bull_divergence(
    ohlc: pd.DataFrame,
    dif: pd.Series,
    hist: pd.Series,
    *,
    ...
    """Scan for DEA bullish-cycle slope-reversal bottom signals.
    ...
    lookback : bars to search backward for the DEA pullback trough.
    ...
    hist_scale : histogram scaling factor (hist = hist_scale * (dif − dea)).
    """
    ...
    for i in range(min_lookback, n):
        ...
        # DEA trough: minimum DEA in window (pullback low within bullish phase)
        dea_window = dea_arr[effective_start:i]
        dea_window_clean = np.where(np.isfinite(dea_window), dea_window, np.inf)
        trough_rel = int(np.argmin(dea_window_clean))
        trough_abs = effective_start + trough_rel
        trough_dea = dea_arr[trough_abs]

        # In bullish DIF cycle DEA is typically positive; skip if not finite or negative
        ...
        # Amplitude: slope-based (DEA slope grows positive in bullish recovery,
        # so abs-slope decay would often be 0 — use slope depth vs recovery instead)
        ...
        signals.append(
            DivergenceSignal(
                ...
                amplitude_side=AmplitudeSide(
                    ...
                    candidate_value=dea_slope_arr[i],
